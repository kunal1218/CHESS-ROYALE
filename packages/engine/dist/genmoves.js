...
import { at, coordFromWorld, toWorldX, inBounds } from "./board.js";
...
export function genPseudo(state, from) {
    const b = state.board;
    ...
    if (flags.disco) {
        if (isPawn(p)) {
            ...
            if (forward) {
                ...
                if (cell?.piece && colorOf(cell.piece) === theirs)
                    out.push(forward);
            }
            return out;
        }
    ...
    if (isKing(p)) {
        ...
        if (rights?.kingSide)
            maybePushCastle(out, b, from, me, +1);
        if (rights?.queenSide)
            maybePushCastle(out, b, from, me, -1);
    }
    if (isPawn(p)) {
        ...
        for (const dx of [-1, +1]) {
            ...
            if (targetCell?.piece && colorOf(targetCell.piece) === theirs)
                out.push(target);
        }
        ...
        if (ep) {
            for (const dx of [-1, +1]) {
                const target = coordFromWorld(b, baseWorldX + dx, from.y + forwardDir);
                if (!target || target.x !== ep.x || target.y !== ep.y)
                    continue;
...
function maybePushCastle(out, b, from, color, dir) {
    const step2 = from.x + dir * 2;
    if (!inBounds(b, step2, from.y))
        return;
    const rookX = dir === 1 ? b.width - 1 : 0;
    for (let x = from.x + dir; x !== rookX; x += dir) {
        if (!inBounds(b, x, from.y))
            return;
    ...
    if (!rookCell?.piece || !isRook(rookCell.piece) || colorOf(rookCell.piece) !== color)
        return;
    out.push({ x: step2, y: from.y });
}
function isPrinceMergeTarget(b, x, y, color) {
    const cell = at(b, x, y);
    if (!cell?.piece || colorOf(cell.piece) !== color)
        return false;
    if (!isPawn(cell.piece))
        return false;
