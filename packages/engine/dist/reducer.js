        ...
        case "MOVE": {
            ...
            if (isRook(movingPiece)) {
                ...
                if (a.from.y === startRank) {
                    if (a.from.x === 0)
                        castlingForMover.queenSide = false;
                    if (a.from.x === prev.board.width - 1)
                        castlingForMover.kingSide = false;
                }
            }
            ...
            if (captured && isRook(captured)) {
                ...
                if (a.to.y === enemyStartRank) {
                    if (a.to.x === 0)
                        enemyRights.queenSide = false;
                    if (a.to.x === prev.board.width - 1)
                        enemyRights.kingSide = false;
                }
            }
            ...
            if (pending?.length) {
                const { board, flags, followUp, messages: eventMessages } = processScheduledEvents({ board: next.board, flags: next.flags }, pending);
                next.board = board;
                next.flags = flags;
                ...
                if (followUp.length) {
                    ...
                    (_a = next.schedule)[targetMove] ?? (_a[targetMove] = []);
...
function maybePromote(board, to, moverColor, choice) {
    const cell = at(board, to.x, to.y);
    if (!cell?.piece || !isPawn(cell.piece))
        return;
    if (cell.pieceFlags?.restrictions?.noPromotion)
        return;
    if (!isPromotionRank(board, to, moverColor))
        return;
...
function isPromotionRank(board, square, color) {
    return color === "w" ? square.y === 0 : square.y === board.height - 1;
}
function normalizePromotion(choice) {
    switch (choice) {
        case "N":
        case "B":
        case "R":
...
function promotionId(color, choice) {
    switch (choice) {
...
function scheduleEarthquake(next, prev) {
    ...
    const quakeEvent = { t: "EARTHQUAKE", row, direction, duration };
    ...
    const restoreEvent = { t: "RESTORE_ROW", row };
...
function scheduleDisco(next, prev) {
    const delay = 3;
    const duration = 5;
    const startMove = prev.moveCount + delay;
    const startEvent = { t: "DISCO_START", duration };
...
function appendMessage(state, text, kind) {
...
function pruneExpiredMessages(state) {
    state.messages = state.messages.filter(msg => {
        if (msg.kind === "warning" && state.flags.disco)
            return false;
        if (msg.kind === "danger" && !state.flags.disco)
            return false;
        return true;
    });
}
function determineQuakeRows(height) {
    if (height <= 0)
        return [];
