...
import { at, Board, Coord, coordFromWorld, toWorldX } from "./board.js";
...
export function isSquareAttacked(
  ...
  sq: Coord,
  by: Color,
  flags: Pick<GameFlags, "disco">
): boolean {
  if (flags.disco) return isSquareAttackedDisco(b, sq, by);
  ...
  for (const [dx, dy] of DIRS.knight) {
    ...
    if (p && colorOf(p) === by && (isKnight(p) || isPrince(p))) return true;
  }
  for (const [dx, dy] of DIRS.rook) {
    const hit = firstBlockerOnRay(b, sq.x, sq.y, dx, dy);
    if (hit) { const p = hit.cell.piece!; if (colorOf(p) === by && (isRook(p) || isQueen(p))) return true; }
  }
  for (const [dx, dy] of DIRS.bishop) {
    const hit = firstBlockerOnRay(b, sq.x, sq.y, dx, dy);
    if (hit) { const p = hit.cell.piece!; if (colorOf(p) === by && (isBishop(p) || isQueen(p))) return true; }
  }
  for (let dx=-1; dx<=1; dx++) for (let dy=-1; dy<=1; dy++) if (dx||dy) {
    const origin = coordFromWorld(b, targetWorldX - dx, sq.y - dy);
    if (!origin) continue;
    const p = at(b, origin.x, origin.y)?.piece;
    if (p && colorOf(p) === by && isKing(p)) return true;
  }
  ...
  for (const dx of [-1, +1]) {
    ...
    if (!origin) continue;
    const p = at(b, origin.x, origin.y)?.piece;
    if (p && colorOf(p) === by && (isPawn(p) || isPrince(p))) return true;
  }
  return false;
}

function isSquareAttackedDisco(b: Board, sq: Coord, by: Color): boolean {
  const targetWorldX = toWorldX(b, sq);
  ...
  for (const [dx, dy] of DIRS.rook) {
    ...
    if (hit) {
      ...
      if (piece && colorOf(piece) === by && isKing(piece)) return true;
    }
  }
  for (const [dx, dy] of DIRS.bishop) {
    const hit = firstBlockerOnRay(b, sq.x, sq.y, dx, dy);
  ...
  for (let dx = -1; dx <= 1; dx++) {
    for (let dy = -1; dy <= 1; dy++) {
      ...
      if (piece && colorOf(piece) === by && isQueen(piece)) return true;
    }
  }
  ...
  if (origin) {
    const piece = at(b, origin.x, origin.y)?.piece;
    if (piece && colorOf(piece) === by && isPawn(piece)) return true;
  }
  return false;
}
